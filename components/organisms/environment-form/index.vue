<template>
  <v-dialog :value="visible" max-width="720px" @click:outside="hideDialog">
    <v-card>
      <v-card-title>
        <span class="text-h5">Create new publishing environment</span>
      </v-card-title>
...
<script>
  export default {
    ...
    methods: {
      ...
      async attemptCreate() {
        if (!this.name) {
          return this.$store.commit('alert/set', { message: `You have to give the environment a name.`, type: 'error' });
        }
        if (!this.key) {
          return this.$store.commit('alert/set', { message: `You have to give the environment a key.`, type: 'error' });
        }
        ...
        try {
          ...
          const { error } = await this.$api('/publishing-environments', {
            method: 'POST',
            headers: {
          ...
          this.$store.commit('alert/set', { message: 'Publishing environment successfully created!', type: 'success' });
