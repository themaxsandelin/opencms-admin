<template>
  <v-dialog :value="visible" max-width="720px">
    <v-card>
      <v-card-title>
        <span class="text-h5">Create new question variant</span>
      </v-card-title>
  ...
  export default {
    ...
    methods: {
      ...
      async attemptCreateVariant() {
        ...
        const response = await this.$api(`/content-blocks/${this.$route.params.questionId}/variants`, {
          method: 'POST',
          headers: {
        ...
        this.$store.commit('alert/set', { message: 'Question variant successfully created!', type: 'success' });
        this.name = '';
        this.hideForm();
        this.$emit('created');
      }
