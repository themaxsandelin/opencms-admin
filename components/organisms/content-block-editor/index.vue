...
// Components
  ...
  export default {
    ...
    methods: {
      ...
      selectLatestVersion() {
        this.versionSelection(this.versions.length ? this.versions[0].id : this.getBaseVersionData().id);
      },
      publishChange(data) {
      ...
      versionSelection(versionId) {
        ...
        if (version) {
          ...
          this.$set(this.$data, 'publishToEnvironments', this.selectedEnvironments);
        }
      },
      async updateLocales() {
      ...
      async publish() {
        const failed = [];
        await Promise.all(
          this.publishToEnvironments
            .filter((environment) => !this.selectedEnvironments.includes(environment))
          ...
          this.selectedEnvironments
            .filter((environment) => !this.publishToEnvironments.includes(environment))
            .map(async (environment) => {
              const { error } = await this.$api(`/content-blocks/${this.block.id}/variants/${this.variant.id}/versions/${this.selectedVersion.id}/unpublish`, {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json'
                },
